...
const DestinationsGrid = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['destinations', debouncedSearch, category, sortOrder],
    queryFn: async () => {
               ...
               <div className="flex flex-col space-y-4 w-full sm:w-auto">
                 ...
                 <Select value={sortOrder} onValueChange={(val) => setSortOrder(val || 'asc')}>
                  ...
                  <SelectContent className="bg-background border-border/60 rounded-2xl shadow-2xl">
                    <SelectItem value="asc" className="font-bold py-3">Price: Low to High</SelectItem>
                    <SelectItem value="desc" className="font-bold py-3">Price: High to Low</SelectItem>
                  </SelectContent>
                </Select>
               </div>
            </div>
